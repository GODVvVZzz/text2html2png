...
- The shared layout realigned with the original density: every theme renders on the 860px content column (908px viewport) with tighter card gaps and padding, theme title sizes drop from 31-38px back to the original 22-32px scale, and lede copy follows each theme's original secondary size. The org-chart node cards were tightened to match, and bilingual example copy was shortened where the narrower column left orphan words on the last line.
