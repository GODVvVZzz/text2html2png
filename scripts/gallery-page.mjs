...
<meta property="og:title" content="text2html2png — ${w.title}"><meta property="og:description" content="${w.intro}"><meta property="og:image" content="https://raw.githubusercontent.com/GODVvVZzz/text2html2png/main/assets/social-card.png"><meta name="twitter:card" content="summary_large_image">
...
<section class="proof"><h2>${w.proof}</h2><div class="proof-items">${w.proofs.map(([title, text]) => `<article><h3>${title}</h3><p>${text}</p></article>`).join('')}</div><p class="boundary">${w.boundary}</p></section>
