...
import { loadExamples, coverageReport } from "../skills/text2html2png/scripts/examples.mjs";
...
function card(example) {
  return `      <article class="card">
        <a class="shot" href="gallery/${example.id}.png">
          <img src="gallery/${example.id}.png" alt="${escapeHtml(example.title)}, a ${escapeHtml(example.chart)} diagram in the ${escapeHtml(example.style)} theme" loading="lazy">
        </a>
        <div class="body">
          <div class="tags">
            <span class="tag chart">${escapeHtml(CHART_LABELS[example.chart] ?? example.chart)}</span>
            <span class="tag theme">${escapeHtml(example.style)}</span>
          ...
          <div class="links">
            <a href="${BLOB_BASE}/skills/text2html2png/${example.htmlRelative}">Editable HTML</a>
            <a href="${BLOB_BASE}/skills/text2html2png/${example.pngRelative}">Full-resolution PNG</a>
          </div>
        </div>
      </article>`;
}
...
function page(examples, coverage) {
  ...
  const styles = 7 - coverage.missingStyles.length;
...
function promptsMarkdown(examples, coverage) {
  ...
  for (const example of sorted) {
    lines.push(`| ${example.title} | ${CHART_LABELS[example.chart] ?? example.chart} | \`${example.style}\` | [HTML](${example.id}.html) · [PNG](../${example.pngRelative}) |`);
  }
  ...
  for (const example of sorted) {
    lines.push(
      ...
      `## ${example.title}`,
      "",
      `\`${example.chart}\` × \`${example.style}\` · rendered at ${example.width}px`,
      "",
      `> ${example.prompt}`,
      "",
      example.summary,
      "",
      `Output: [HTML](${example.id}.html) · [PNG](../${example.pngRelative})`,
    );
  }
...
async function main() {
  ...
  if (!skipRender) {
    ...
    for (const example of examples) {
      // Scale 1 keeps the published page fast; the full-resolution PNG stays in the skill.
      await renderScreenshot({
        html: example.htmlPath,
        out: path.join(docsGalleryDir, `${example.id}.png`),
      ...
      console.log(`rendered docs/gallery/${example.id}.png`);
    }
  }
